...
interface Props<TItem, TValue extends Key = string> {
  values?: TItem[];
  onValueChange?: (values: TItem[]) => void;
  options: TItem[];
  idAccessor: (item: TItem) => TValue;
  labelAccessor: (item: TItem) => ReactNode;
  placeholder?: string;
  id?: string;
  truncateLength?: number;
}

export function MultiCombobox<TItem, TValue extends Key = string>({
  values,
  options,
  onValueChange,
  idAccessor,
    ...
    >
      ...
      <BCombobox.Portal>
        <BCombobox.Positioner
          ...
          anchor={containerRef}
        >
          <BCombobox.Popup className="dark:-outline-offset-1 max-h-[min(var(--available-height),23rem)] w-[var(--anchor-width)] max-w-[var(--available-width)] origin-[var(--transform-origin)] scroll-pt-2 scroll-pb-2 overflow-y-auto overscroll-contain rounded-md bg-card py-2 text-card-foreground shadow-gray-200 shadow-lg outline-1 outline-border transition-[transform,scale,opacity] data-[ending-style]:scale-95 data-[starting-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 dark:shadow-none">
            ...
            <BCombobox.List>
              {(option: TItem) => (
                ...
                >
                  ...
                  <div className="col-start-2">{labelAccessor(option)}</div>
                </BCombobox.Item>
              )}
            </BCombobox.List>
          </BCombobox.Popup>
